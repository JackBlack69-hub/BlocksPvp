...
import { Drawer,Paper, List, ListItem, Button, TextField, IconButton, Avatar, Typography } from '@mui/material';
...
function RightMenu({ open, onClose }) {
  ...
  return (
    ...
    >
      ...
      <List style={{ marginTop: 'auto', marginBottom: '10px', maxHeight: 'calc(100vh - 200px)', overflowY: 'auto' }}>
        {chatMessages.map((message, index) => (
          <ListItem key={index}>
            <Paper style ={{display:'flex',padding:10, flexDirection:'row',height:100, alignItems:'center',width:'100%', borderRadius:20}}>
      ...
      <List>
        <ListItem>
          <TextField
            ...
            style={{ backgroundColor: '#2B293D', color: '#FFFFFF', borderRadius: '15px', border: '2px solid pink' }}
            InputProps={{
              style: { color: '#FFFFFF' },
              endAdornment: (
                <IconButton edge="end">
